...
public class FirstCharGrouping implements GroupingStrategy, Serializable {
  private static final long serialVersionUID = -1121182295793347602L;

  public FirstCharGrouping() {
  }
  ...
  public int getInstance(Event event, int parallelism) {
    String eventData = getKey(event).toString();
    Character firstChar = Character.toLowerCase(eventData.charAt(0));
    return (firstChar - 'a') % parallelism;
    }
}
